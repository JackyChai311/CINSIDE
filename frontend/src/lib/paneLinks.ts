 ...
 * 返回 true=已互换（需镜像）、false=未互换、null=无法判定（目标网站都没开，调用方回退其他依据）
 */
export function sideFrameByOrigins(
  ...
  if (recL) {
    if (cur.left.includes(recL)) hits++;
    else if (cur.right.includes(recL)) { vote++; hits++; }
  }
  if (recR) {
    if (cur.right.includes(recR)) hits++;
    else if (cur.left.includes(recR)) { vote++; hits++; }
  }
...
export function checkGroupPanes(
  ...
  for (const side of ["left", "right"] as const) {
    ...
    const there = cur[side === "left" ? "right" : "left"].includes(p.origin);
    if (!here && !there) {
